...
import fetch_free_proxyes, get_proxy_from_db
...
class HttpProxyMiddleware(object):
    ...
    def __init__(self, use_https):
        ...
        self.extend_proxy_threshold = 50
        # 初始化代理列表
        ...
        # 从db取初始代理
        self.get_init_proxies()

    def get_init_proxies(self):
        _type = 'http'
        if self.uses_https:
            _type = 'https'
        _proxies = get_proxy_from_db.push_new_proxies(300, _type)
        for proxy in _proxies:
            if self.url_in_proxyes(proxy):
                continue
            self.proxyes.append({"proxy": proxy,
                                 "valid": True,
                                 "count": 0})
